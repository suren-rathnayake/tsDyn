...
### Obtain infos for the two thresh models
  set1 <- setar(x, m, d=d, steps=steps, thDelay=thDelay, th=firstBests["th"], 
                trace=FALSE, nested=FALSE, include = c("const", "trend","none", "both"), 
                common="none", model=c("TAR", "MTAR"), nthresh=1, trim=trim)
  
  set2 <- setar(x, m, d=d, steps=steps, thDelay=thDelay, th=bests[c("th1", "th2")], 
                trace=FALSE, nested=FALSE, include = c("const", "trend","none", "both"), 
  ...
  res <- list(Ftests = Ftests,
              ...
              PvalBoot = PvalBoot,
              Ftestboot = Ftestboot,
              nboot = nboot, args = args, Bound = Bound,
              updated = NULL)
