...
#'  vecm1 <- TVECM.sim(B=Bvecm, nthresh=0, beta=1,n=100, lag=1,include="none", innov=innov, starting=startVal)
#'  ECT <- vecm1[,1]-vecm1[,2]
#'
#'#add an intercept as in panel B
#'  Bvecm2 <- rbind(c(-0.2, 0.1,0,0), c(0.2,0.4, 0,0))
#'  vecm2 <- TVECM.sim(B=Bvecm2, nthresh=0, n=100,beta=1, lag=1,include="const", innov=innov, starting=startVal)
#'
#'  par(mfrow=c(2,1))
...
#'
#'##Check the bootstrap
#' TVECM.sim.check <- TVECM.sim(TVECMobject=TVECM(dat, nthresh=1, lag=2, ngridBeta=20, ngridTh=20, plot=FALSE, include="none"),type="check")
